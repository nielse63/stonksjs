...
const parseBars = (barsArray) => {
  ...
  const maFast = series
    ...
    .ema(SMA_INTERVALS.fast);
  const maSlow = series
    .deflate((bar) => bar.close)
    .ema(SMA_INTERVALS.slow);

  return series
    .withSeries('maFast', maFast)
    .skip(SMA_INTERVALS.fast)
    .withSeries('maSlow', maSlow)
...
const strategy = {
  entryRule: (enterPosition, args) => {
    if (args.bar.maFast > args.bar.maSlow) { // Buy when price is below average.
      enterPosition();
    }
  },
  exitRule: (exitPosition, args) => {
    if (args.bar.maFast < args.bar.maSlow) {
...
const runBacktest = (barsObject, startingCapital) => {
  const output = Object.entries(barsObject).reduce((object, [symbol, bars]) => {
    ...
    const shouldBuy = maFast > maSlow && analysis.profitPct > 20;
    return {
      ...object,
      [symbol]: {
        bars,
